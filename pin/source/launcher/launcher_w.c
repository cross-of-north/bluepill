    ...
    char** user_argv)
{
     ...
     */
    child_argv[child_argv_ind++] = escape_string(appendPath(base_path, "/", "ia32/bin/pin.exe"));
    child_argv[child_argv_ind++] = "-p64";
    child_argv[child_argv_ind++] = escape_string(appendPath(base_path, "/", "intel64/bin/pin.exe"));
...
void set_environment(char* base_path)
{
    ...
    char* tzdata = (char*)malloc(strnlen_s(base_path, MAX_PATH) + 30);
    char* tzdata_path = appendPath(base_path, "/", "extras/crt/tzdata");
    tzdata = escape_string(tzdata_path);
